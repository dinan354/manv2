    ...
    <div class="row">
      ...
      <div class="col-md-2">
        @if($current->myrole[0]->id == 1)
          @if($blog->status == 0)
            <a href="{{ url('/weblog/activate/') }}/{{ $blog->id }}" class="btn btn-primary">Tambahkan ke web</a>
          @else
            <a href="{{ url('/weblog/deactivate/') }}/{{ $blog->id }}" class="btn btn-primary">Hapus dari web</a>
          @endif
        @else
        @endif
      </div>
