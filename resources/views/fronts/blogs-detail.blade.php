@extends('layouts.app')
@section('title', 'Blog')
@section('keywords', 'Blog MAN Balikpapan')
@section('breadcrumbs')
 <!-- Page -->
  ...
  <div class="blog-post">
    <div class="blog-header">
      <!--Big Date-->
      <div class="blog-date">
        <span class="month">{{ date("F", mktime(0, 0, 0, Carbon::parse($ns->updated_at)->month, 10)) }}</span>
        <span class="day">{{ Carbon::parse($ns->updated_at)->day }}</span>
        <span class="year">{{ Carbon::parse($ns->updated_at)->year }}</span>
      </div>
      <!--/Big Date-->
...
<script>
    ...
    var disqus_config = function () {
        this.page.url = "{{ url('/blog'.$ns->slug) }}";  // Replace PAGE_URL with your page's canonical URL variable
        this.page.identifier = "{{ $ns->slug }}"; // Replace PAGE_IDENTIFIER with your page's unique identifier variable
    };
    
    (function() {  // DON'T EDIT BELOW THIS LINE
        var d = document, s = d.createElement('script');
        
        s.src = '//manblpp.disqus.com/embed.js';
